...
export const OldContestSubmissionForm = ({
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        if (response.ok && result.success && result.data) {
          ...
          if (!initialData?.categoryId && result.data.length > 0) {
    ...
    fetchCategories();
  }, [getToken, initialData?.categoryId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (!categoryId || !firstName || !lastName || !title) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        {isLoadingCategories ? (
          ...
          <div className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-red-400">
