...
const LATEST_JUDGES_FILE = join(DATA_DIR, 'latest-judges-contest.ts');
...
`;
  ...
  // 8. Save the newest contest that actually has judges.
  // `finalOutput` is sorted year-descending, so the first hit is the newest.
  // This lives in its own module because the navbar links to it and the navbar
  // is client-hydrated — importing past-contests.ts there would ship every
  // contest and entry to the browser.
  const latestJudgesContest =
    finalOutput.find(c => c.judges.length > 0) ?? finalOutput[0];

  const latestJudgesContent = `// Auto-generated by scripts/export-past-contests.ts
// Do not edit manually
...
export const latestJudgesContestId = '${latestJudgesContest.id}';
`;

  const tempLatestTs = `${LATEST_JUDGES_FILE}.tmp`;
  writeFileSync(tempLatestTs, latestJudgesContent);
  renameSync(tempLatestTs, LATEST_JUDGES_FILE);
  ...
  console.log(
    `⚖️  Latest judges contest: ${latestJudgesContest.id} -> ${LATEST_JUDGES_FILE}`
  );
}
